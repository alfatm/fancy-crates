...
# Validate Cargo.toml using sparse-crates CLI
echo -e "\n\n[+] Running sparse-crates validation..."
npm run build:cli
node out/cli.cjs test-workspace/Cargo.toml \
  --registry public-registry=http://localhost:8000/api/v1/crates/ \
  --registry private-registry=http://localhost:8001/api/v1/crates/
